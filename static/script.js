...
const THEMES = [
  ["🎌", "Anime"], ["📺", "Séries"], ["🎬", "Filmes"], ["🧙", "Personagens"],
  ["🎮", "Jogos"], ["🐾", "Animais"], ["⚽", "Esportes"], ["🍕", "Comidas"],
  ["💻", "Tecnologia"], ["🌎", "Países e Lugares"], ["🔬", "Ciência"], ["🎵", "Música"],
  ["🧑‍🔧", "Profissões"], ["🌳", "Natureza"], ["📚", "Livros"], ["🦸", "Super-heróis"],
  ["🏫", "Escola"], ["✨", "Outros"]
];
...
const LANGUAGES = [
  ...
  ["zh","🇨🇳 中文"],["ru","🇷🇺 Русский"],["uk","🇺🇦 Українська"],["el","🇬🇷 Ελληνικά"],
...
const state = {
  mode:"random", theme:"Anime", difficulty:2, language:"pt", word:"", hints:[],
  hintCount:0, errors:0, score:0, streak:0, round:1, correct:0,
  timeLeft:Infinity, timer:null, guessed:new Set(), database:[],
  unlocked:false, finished:false, staticSite:/\\.github\\.io$/.test(location.hostname)
};
...
function guessable(char) { return /[\\p{L}\\p{N}]/u.test(char); }
function keyOf(char) { return normalizeChar(char); }

function saveSettings() {
  try {
    localStorage.setItem("forca-settings", JSON.stringify({
      time:$("time").value, language:state.language, player:$("player").value,
      theme:state.theme, difficulty:state.difficulty
    }));
  } catch (_) {}
}
...
function loadSettings() {
  try {
    ...
    if (!saved) return;
    if (saved.language && LANGUAGES.some(function(item){return item[0]===saved.language;})) state.language=saved.language;
    if (Number.isInteger(saved.difficulty) && saved.difficulty>=0 && saved.difficulty<=6) state.difficulty=saved.difficulty;
    if (THEMES.some(function(item){return item[1]===saved.theme;})) state.theme=saved.theme;
    if (saved.player) $("player").value=saved.player;
  } catch (_) {}
}

function screen(id) {
  ["menu","config","game","result"].forEach(function(item){$(item).classList.toggle("hidden",item!==id);});
  $("status").textContent=id==="game"?"Jogando":id==="config"?"Configuração":id==="result"?"Resultado":"Menu";
  window.scrollTo({top:0,behavior:"smooth"});
}

function languageOptions() {
  const html=LANGUAGES.map(function(item){return '<option value="'+item[0]+'">'+item[1]+'</option>';}).join("");
  [$("lang"),$("from"),$("to")].forEach(function(select){select.innerHTML=html;});
...
function timeOptions() {
  const list=[[30,"30 segundos"],[60,"1 minuto"],[120,"2 minutos"],[180,"3 minutos"],[300,"5 minutos"],
    [600,"10 minutos"],[900,"15 minutos"],[1200,"20 minutos"],[1800,"30 minutos"],[3600,"60 minutos"],["inf","Tempo infinito"]];
  $("time").innerHTML=list.map(function(item){return '<option value="'+item[0]+'">'+item[1]+'</option>';}).join("");
  $("time").value="180";
  try {
    const saved=JSON.parse(localStorage.getItem("forca-settings")||"null");
    if(saved && saved.time && Array.from($("time").options).some(function(o){return o.value===String(saved.time);})){
      $("time").value=String(saved.time);
    }
  } catch (_) {}
}

function renderOptions() {
  $("themes").innerHTML=THEMES.map(function(item){
    return '<button type="button" class="opt '+(state.theme===item[1]?"selected":"")+
      '" data-theme="'+escapeHTML(item[1])+'">'+item[0]+" "+escapeHTML(item[1])+
      (state.theme===item[1]?" ✓":"")+"</button>";
  }).join("");

  $("diffs").innerHTML=DIFFICULTIES.map(function(name,index){
    return '<button type="button" class="opt '+(state.difficulty===index?"selected":"")+
      '" data-diff="'+index+'">'+(index+1)+". "+name+(state.difficulty===index?" ✓":"")+"</button>";
  }).join("");
}
  ...
  try {
    ...
    if(Array.isArray(raw)) {
      ...
      state.database=Object.keys(raw.themes).reduce(function(all,theme){
        ...
        return all.concat(items.map(function(item){
...
async function randomWord() {
  const query=new URLSearchParams({
    theme:state.theme, language:state.language, difficulty:String(difficultyLimit())
  });

  if(!state.staticSite) {
    try {
      const response=await fetch("api/word?"+query.toString(),{cache:"no-store"});
      if(response.ok) {
  ...
  if(!pool.length) {
    pool=db.filter(function(item){return Number(item.difficulty||3)<=difficultyLimit()+1;});
  }

  if(!pool.length) {
    return {
      word:"Python",theme:"Tecnologia",difficulty:1,language:"pt",
      hints:["É uma linguagem de programação.","É usada em automação e dados.","Também é o nome de uma cobra em inglês."]
    };
  }
...
function friendHints() {
  return [$("friendHint1").value.trim(),$("friendHint2").value.trim(),$("friendHint3").value.trim()]
...
async function startGame() {
  ...
  $("adminControls").classList.add("hidden");$("adminPass").value="";
  saveSettings();
...
function autoHints() {
  return state.correct>=6?3:state.correct>=4?2:state.correct>=2?1:0;
}

function visibleHints() {
  return Math.min(3,Math.max(state.hintCount,autoHints()));
}
...
function renderGame() {
  ...
  const hintCount=visibleHints();
  $("hints").innerHTML=state.hints.map(function(hint,index){
    return '<div class="hint '+(index<hintCount?"":"locked")+'"><b>💡 Dica '+(index+1)+"</b>"+
      (index<hintCount?escapeHTML(hint):"🔒 Liberada conforme você joga")+"</div>";
  }).join("");

  $("word").innerHTML=chars(state.word).map(function(char){
    if(!guessable(char)) return "<span>"+escapeHTML(char)+"</span>";
    return "<span>"+(state.guessed.has(keyOf(char))?escapeHTML(char):"_")+"</span>";
  }).join(" ");
...
function solved() {
  return chars(state.word).filter(guessable).every(function(char){
    return state.guessed.has(keyOf(char));
  });
}
...
function configuredTime() {
  return $("time").value==="inf"?Infinity:Number($("time").value);
}

function runTimer() {
  clearInterval(state.timer);
  state.timeLeft=configuredTime();
  updateTimer();
...
function updateTimer() {
  $("timer").textContent=state.timeLeft===Infinity?"∞":Math.max(0,state.timeLeft)+"s";
}
  ...
  if(!state.staticSite) {
    try {
      ...
      if(response.ok) {
        ...
        $("trResult").value=result.text||text;
        $("translateStatus").textContent="Usando o backend Python.";
        return;
      }
    } catch (_) {}
  }
...
function localTranslate(text,source,target) {
  const sourceTable=TR[source]||{},targetTable=TR[target]||{};
  const normalized=text.toLocaleLowerCase();
  const concept=Object.keys(sourceTable).find(function(key){
    return sourceTable[key].toLocaleLowerCase()===normalized;
  });
...
async function unlockAdmin() {
  ...
  if(state.staticSite) {
    ...
    alert("O painel Admin protegido pelo backend não funciona no GitHub Pages. Rode o app.py para usar essas ferramentas.");
    return;
  }
...
function addTime(seconds) {
  if(!admin())return;
  if(state.timeLeft!==Infinity)state.timeLeft+=seconds;
  updateTimer();
}

function revealWord() {
  if(!admin())return;
  chars(state.word).filter(guessable).forEach(function(char){state.guessed.add(keyOf(char));});
  state.hintCount=3;
  renderGame();
}

function removeError() {
  if(admin()&&state.errors>0){state.errors-=1;renderGame();}
}

function nextHint() {
  if(admin()){state.hintCount=Math.min(3,state.hintCount+1);renderGame();}
}

function addScore() {
  if(admin()){state.score+=100;renderGame();}
}

function resetRound() {
  if(!admin())return;
  state.errors=0;state.correct=0;state.hintCount=0;state.guessed.clear();state.finished=false;
  renderGame();runTimer();
}
...
$("themes").addEventListener("click",function(event){
  ...
  if(button){state.theme=button.dataset.theme;renderOptions();saveSettings();}
});

$("diffs").addEventListener("click",function(event){
  const button=event.target.closest("[data-diff]");
  if(button){state.difficulty=Number(button.dataset.diff);renderOptions();saveSettings();}
});
...
$("back").addEventListener("click",function(){screen("menu");});
$("start").addEventListener("click",startGame);
$("guessBtn").addEventListener("click",handleInput);
$("guessInput").addEventListener("keydown",function(event){if(event.key==="Enter")handleInput();});
...
if(state.staticSite) $("status").title="Executando como site estático no GitHub Pages";
